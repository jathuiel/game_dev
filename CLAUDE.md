...
Jogo 2D de aventura top-down, pixel art. Plataforma atual: **HTML5/web mobile** (protótipo em `web/`). Projeto Godot 4 em `godot/` está **em espera**.
...
- `assets/personagem2/` — segundo herói jogável ("Super Operário"), mesmo pipeline genérico apontado para uma segunda folha de referência (`extrair_super_operario.py`, `padronizar_super_operario.py`, `gerar_animacoes_operario.py`, `reduzir_16bit_operario.py`). Referenciado em `web/js/assets.js` e na tela de seleção (`web/screens/personagem.js`).
